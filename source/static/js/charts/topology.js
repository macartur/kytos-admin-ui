var topology = (function(){
  ...
  var container_id = '#topology-chart';
  ...
  function get_current_layout() {
    ...
    layout.other_settings['topology_transformation'] = d3.select(container_id + ' svg g').attr('transform');
    return layout;
  }
  ...
  function appendLayoutListItem(item){
    ...
    if (!exists){
      $('<li><a href="#" data-value="'+ item +'" onclick="topology.layouts.restore(\''+ item +'\')">'+ item +'</a></li>').appendTo(savedLayouts);
    }
  }
  ...
  var width = $(container_id).parent().width(),
  ...
  // Switches
  api.switches.all = get_switches;
  api.switches.by_dpid = get_switch_by_dpid;
  api.switches.context = show_context;

  // Switch interfaces
  api.interfaces.unused = {};
  api.interfaces.unused.toggle = toggle_unused_interfaces;

  // Hosts
  api.hosts.disconnected = {};
  api.hosts.disconnected.toggle = toggle_disconnected_hosts;

  // Chart methods
  api.draw = draw_topology;
  api.show = function(){$(container_id).fadeIn();}
  api.hide = function(){$(container_id).fadeOut();}
  api.toggle = function(){$(container_id).toggle(400);}
  api.labels = {};
  api.labels.toggle = toggle_labels;

  // Layout methods
  api.layouts = {};
  api.layouts.load = load_layouts;
  api.layouts.save = save_layout;
  api.layouts.restore = restore_layout;
